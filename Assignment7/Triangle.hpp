...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    Vector3f E1 = v1 - v0, E2 = v2 - v0, S1 = crossProduct(ray.direction, E2), S = ray.origin - v0, S2 = crossProduct(S, E1);

    Vector3f result = (1 / dotProduct(S1, E1)) * Vector3f(dotProduct(S2, E2), dotProduct(S1, S), dotProduct(S2, ray.direction));

    float x = result.x, y = result.y, z = result.z;

    if (x > 0 && y > 0 && z > 0 && 1 - y - z > 0) {
        inter.distance = x;
        inter.coords = (1 - y - z) * v0 + y * v1 + z * v2;
        inter.happened = true;
        inter.normal = normal;
        inter.m = m;
        inter.obj = this;
